...
function draw_single_cpu_core(coreN)
    ...
    ring_anticlockwise(S.cpu.x, S.cpu.y, coreN.radius, coreN.thickness, coreN.begin_angle, coreN.end_angle, val, coreN.max_value, color_frompercent(tonumber(val)))
        ...
        -- スレッド数の表示
    local threads = running_threads()  -- 実行中のスレッド数を取得
    write(160, 245, "Threads: " .. threads, 12, main_text_color)  -- スレッド数の表示位置を調整
...
function draw_memory()
    ...
    -- 使用中メモリ、最大メモリ、キャッシュメモリ、バッファメモリの表示
    write(S.mem.text.process_title.x, S.mem.text.process_title.y - 18, usedmem, 12, main_text_color)
    -- write(S.mem.text.process_title.x + 30, S.mem.text.process_title.y - 54, "Max:     " .. maxmem, 12, main_text_color)
...
function draw_net()
    ...
    write(S.net.indicators.down.x+25, S.net.indicators.down.y, "▼ ".. download_speed(), 12, main_text_color)
    write(S.net.indicators.up.x+25, S.net.indicators.up.y, "▲ "..upload_speed(), 12, main_text_color)
